...
public class ArucoDetector : MonoBehaviour
{
    ...
    // 처리 순서: 외부꼭짓점 4점 변환(outputWidth×outputHeight) → 회전 → Fish0 마스크 적용
    // 반환된 Texture2D는 호출자가 Destroy() 책임
    Texture2D Warp(Mat frame, Point2f[][] corners, int[] ids)
    {
        // 외부 꼭짓점(마커 바깥 모서리) 기준으로 워프
        ...
        // 워프 출력 크기: 항상 outputWidth×outputHeight 사용 (fish0 텍스처 임포트 크기와 무관)
    ...
    // ── Fish0 마스크 적용 → Texture2D ──
    // 순서: Fish0 마스크(전체 크기 적용) → 그대로 반환 (crop 없음)
    ...
    // ── fish0.png 전체 → 워프 출력 크기 실루엣 마스크 생성 ──────
    // fish0 전체(마커 포함)를 targetW×targetH로 리사이즈 후 alpha 기반 실루엣 추출
    // 마커는 crop 단계에서 물리적으로 제거되므로 마스크에 포함되어도 무방
    ...
    Mat BuildFishMask(int targetW, int targetH)
    {
        ...
        // fish0 전체를 워프 출력 크기(targetW×targetH)로 리사이즈
